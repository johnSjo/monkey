...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{"if (10 > 1) { false + true; }", "unknown operator: BOOLEAN + BOOLEAN"},
		{`
		if (10 > 1) {
			if (10 > 1) {
				return false + true;
	...
	for _, tt := range tests {
		...
		if !ok {
			t.Errorf("on error object returned. got=%T(%+v)", evaluated, evaluated)
			continue
		}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("four")`, 4},
		{`len("hello world")`, 11},
		{`len(1)`, "argument to 'len' not supported. got=INTERGER"},
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash wrong number of pairs. got=%d", len(result.Pairs))
	}

	for expectedKey, ecexpectedValue := range expected {
		...
		testIntegerObject(t, pair.Value, ecexpectedValue)
	}
}
