...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerInfix(token.SLACH, p.parseInfixExpression)
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	if prefix == nil {
		p.noPrefixParesFnError(p.curToken.Type)
		return nil
	}
	leftExp := prefix()
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	exp.Consequence = p.parseBlockStatment()
	...
	if p.peekTokenIs(token.ELSE) {
		...
		exp.Alternative = p.parseBlockStatment()
	}
...
func (p *Parser) noPrefixParesFnError(t token.TokenType) {
